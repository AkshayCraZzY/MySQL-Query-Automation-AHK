...
;	IniWrite, 1, config.ini, COUNT, count
	...
	{
		IniRead, count,config.ini, COUNT, count
		count++
		;count++
		IniWrite, %count%, config.ini, COUNT, count
		IniRead, pathfound,config.ini, PATH, binpath, %A_SPACE%
		drive := SubStr(pathfound, 1, 1)
	...
	FirstTime:
	{
		...
		UData:
		{
			...
			InputBox, pass, %user% Password, Please enter password of %user% database user.,HIDE , 200, 150,,,,,1234
			IniWrite, %user%, config.ini, USERDATA, username
			IniWrite, %pass%, config.ini, USERDATA, password
	...
	;	Send {WheelDown} 
		...
		;	reload
			...
			;	WinActivate ahk_pid %pid%
				...
				sleep 100
				FileAppend,`ncreate table %TName%(, log.txt
				...
				while (endcount<=tempexec)
				{
					...
					if (tempexec=1)
					{
						...
						sleep 100
						FileAppend,%CoName% %DaType%(%SizeT%) %MoreT%);, log.txt
					}
					else
					{
						if !(tempexec=endcount)
						{
							...
							sleep 100
							FileAppend,%CoName% %DaType%(%SizeT%)%MoreT%%comma%, log.txt
						}
						else
						{
							controlsend,,%CoName% %DaType%(%SizeT%) %MoreT%);`n,ahk_pid %pid%
